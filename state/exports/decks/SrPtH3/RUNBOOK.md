# SrPtH3 (A08) — RTSC DFT el-ph 런북

🧪 **SrPtH3** · 별칭 **스트론튬-백금-수소 페로브스카이트** · SrAuH3 의 B-site Pt 치환 변형 (CaAuH3 파일럿 자매)

B-site 전이금속을 Au → Pt (d-전자 1개 적음) 로 바꿔 페로브스카이트-수소화물의 Tc/안정성이
어떻게 변하는지 검증한다. A-site Sr 은 앵커와 동일. Pt-H 팔면체 골격은 Au-H 와 거의 등구조이나,
Pt 는 5d⁹ → 5d⁸ 영역으로 페르미 준위 부근 DOS 와 EPC 가 달라진다 — 이것이 판별점.
...
- **구조**: ibrav=1 simple cubic, nat=5, ntyp=3 — Sr @ A-site (½,½,½), Pt @ B-site (0,0,0),
  H @ 3개 face-center → Pt-H 팔면체 골격.
- **격자 START**: celldm(1)=7.2340 Bohr (=3.828 Å, SrAuH3 ×1.000). Pt(금속반경 1.39Å) ≈ Au(1.44Å)
  → 거의 등구조, A-site 변화 없음 → 앵커 a 를 그대로 시작값으로. vc-relax 가 진짜 a 를 찾음.
...
|------|-----------|------------|------|
| Sr | `Sr.pbe-spn-rrkjus_psl.1.0.0.UPF` | scalar | semicore 4s,4p,n — SrAuH3 anchor 재사용 (검증됨) |
| Pt | `Pt.pbe-n-rrkjus_psl.1.0.0.UPF` | **scalar-relativistic** | ⚠ 5d — 아래 SOC 노트 |
| H  | `H.pbe-rrkjus_psl.1.0.0.UPF` | — | SrAuH3 anchor 재사용 (검증됨) |

> ⚠ **Pt 는 5d 전이금속 — SOC + 상대론 의사퍼텐셜이 중요하다** (Au 와 동일 클래스).
> baseline 은 **scalar-relativistic** PSL Pt UPF (`Pt.pbe-n-rrkjus_psl.1.0.0.UPF`,
> Au 의 `Au.pbe-n-rrkjus_psl.1.0.0.UPF` 와 동일 `n`(nlcc) 명명규칙). 5d 밴드의 SOC
> 스플리팅이 페르미 준위 부근 DOS 와 EPC 에 영향을 줄 수 있으므로, full-relativistic
> Pt UPF (예: `Pt.rel-pbe-*_psl.1.0.0.UPF`, `lspinorb=.true.`) 가 라이브러리에 있으면
> 그것으로 교차검증한다. scalar 결과를 1차 baseline 으로, full-rel 을 SOC 보정 비교로.

> ⚠ **Sr, H 는 SrAuH3/CaAuH3 anchor 에서 이미 검증된 PSL 1.0.0 명**. 그대로 재사용.
> **Pt 는 실행 직전 반드시 grep 확인할 것 (d13)** — 위 파일명은 PSL 1.0.0 표준 명명규칙
> (Au 와 동일 `n` 변종) 기준. 없으면 pslibrary 에서 받아 스테이징:
> ```bash
> ls -l ~/rtsc_srpth3/pseudo/Pt.pbe-n-rrkjus_psl.1.0.0.UPF || \
>   echo "MISSING_PT_PSEUDO — pslibrary.1.0.0 에서 n(nlcc) 변종 다운로드 필요"
> ```
...
> SrPtH3 = CaAuH3 파일럿과 atom 수(5)·q-grid(nq=8)·metallic 가 **동일** → preflight 출력 동일.
...
vast.ai GPU 팟 (또는 pool ubu-1 free) 에서 아래 체인 실행. workdir = `/home/aiden/rtsc_srpth3`.

```bash
set -e

# 0) 환경 + 워크디렉터리
SRC=/home/aiden/rtsc_srpth3
...
for f in Sr.pbe-spn-rrkjus_psl.1.0.0.UPF \
         Pt.pbe-n-rrkjus_psl.1.0.0.UPF \
...
- falsifier (사전등록): 임의 IBZ q 에서 허수 모드 = closed-negative (이 압력 perovskite Pt 골격 비안정).
...
- **workdir**: `exports/rtsc/decks/SrPtH3/`
