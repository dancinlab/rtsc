# AcBeH8 — RTSC A01 DFT el-ph 런북 (BUILD 산출물 · INFRA-READY)

> 비방사성 Ac 변종(AcBeH8 → AcBeH8). 목표: Be-H clathrate 사전압축(precompressor)이
> Ac 치환 후 저압(0 GPa)에서 보존되는지 — RTSC 압력축 돌파선 검증.
...
| celldm(1) (시작 추정) | **10.0151 Bohr** = a_conv 5.300 Å |
| 격자 도출 | AcBeH8 a_conv ~5.300 Å (저압 앵커) → Ac 0% 수축 (Ac 1.95Å → Ac 1.95Å 반경비) |
| nat | **10** (1 Ac + 1 Be + 8 H) — 1 f.u./primitive |
| ntyp | 3 |
| 목표 압력 | **0 GPa (= 0 kbar)** — AcBeH8 저압 안정성 미러; **fallback 10 GPa (= 100 kbar)** |
| 케이지 | BeH8 clathrate (Be @ 4b, H @ 32f xxx, Ac @ 4a) |
...
### 압력 fallback 절차
0 GPa relax 가 동적 불안정(허수 phonon 다수)이면:
`vc-relax.in` 의 `&CELL press = 0.0` → `press = 100.0` (10 GPa) 으로 바꿔 재실행.
AcBeH8 원논문도 압력↓ 한계가 ~10 GPa 이므로, Ac 변종이 더 높은 압력을 요구할 수 있음.
...
| 원소 | 질량 | UPF 파일명 | 라이브러리 |
|------|------|-----------|-----------|
| Ac | 227.000 | `Ac.pbe-spdfn-rrkjus_psl.1.0.0.UPF` | PSL 1.0.0 (US, PBE) |
...
```bash
#!/usr/bin/env bash
# AcBeH8 RTSC A01 — vc-relax -> scf -> ph(el-ph simple) chain
set -euo pipefail
cd "$(dirname "$0")"
...
> 동적 불안정(허수 freq) → 위 §1 압력 fallback(10 GPa)으로 재발사.
...
| workdir  | ✅ | `exports/rtsc/decks/AcBeH8/` |
